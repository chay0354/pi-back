-- RETURNS TABLE output names collide with table columns. Unqualified
-- `target_subscription_id` in the deactivate-siblings UPDATE made Postgres
-- raise: column reference "target_subscription_id" is ambiguous.
...
BEGIN
  ...
  UPDATE public.agency_marketer_replacement_codes AS replacement_code
  SET is_active = FALSE,
      consumed_at = NOW()
  WHERE replacement_code.id = v_code.id;

  UPDATE public.agency_marketer_replacement_codes AS replacement_code
  SET is_active = FALSE
  WHERE replacement_code.target_subscription_id = v_target.id
    AND replacement_code.id <> v_code.id
    AND replacement_code.is_active = TRUE;
